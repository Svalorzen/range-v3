...
namespace ranges
{
    inline namespace v3
    {
        ...
        public:
            ...
            {
                using _size_t = common_type_t<range_size_t<Rngs const>...>;
                return range_cardinality<iter_zip_with_view>::value >= 0 ?
                    (_size_t)range_cardinality<iter_zip_with_view>::value :
                    tuple_foldl(
                        tuple_transform(
                            rngs_,
                            [](auto&& r) -> _size_t { return ranges::size(r); }),
                        (std::numeric_limits<_size_t>::max)(),
                        detail::min_);
            }
        };
